      ...
      $("#q").keyup(function() {
          ...
          $.getJSON( "/loko/groups/search?q="+$("#q").val(), function( data ) {
            $.each( data, function( key, val ) {
              $('#groups').append("<tr><td>"+val["name"]+"</td><td>"+val["mail"]+"</td><td>"+val["bundesland"]+"</td><td>"+val["aktiv"]+"</td><td>"+val["more"]+"</td><td><a href='/loko/groups/edit?id="+val["id"]+"'>[EDIT]</a><a href='/loko/groups/del?id="+val["id"]+"'>[DEL]</a><a href='https://wiki.junge-piraten.de/wiki/"+val["wiki"]+"' target='_blank'>[WIKI]</a></td></tr>");
              //console.log(val);
            });
          });
      });
